...
import { ClassCompletionToggle } from '@/app/course/[slug]/classes/class-completion-toggle'
...
export async function generateMetadata({ params }: ClassesPageProps): Promise<Metadata> {
...
export default async function CourseClassesPage({ params }: ClassesPageProps) {
	...
	const progress = await CourseClassProgressStorage.listByUserCourse(
		user.uuid,
		course.uuid ?? '',
	)
	const progressLookup = new Map(progress.map(item => [item.classUuid, item.completed]))
	const completedCount = classes.filter(item =>
		progressLookup.get(item.uuid ?? ''),
	).length
	...
	const fileLookup = new Map(filesByClass.map(item => [item.classUuid, item.files]))
	...
	return (
		<PageShell>
			<Section tone="transparent">
				...
				{classes.length ?
					<div className="mb-5 rounded-lg border border-[var(--line)] bg-[var(--surface)] p-4 text-sm text-[var(--muted)]">
						<strong className="text-[var(--ink)]">{completedCount}</strong> of{' '}
						<strong className="text-[var(--ink)]">{classes.length}</strong> classes
						completed
					</div>
				:	null}
							...
							return (
								...
								>
									<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
										...
										<ClassCompletionToggle
											classUuid={item.uuid ?? ''}
											initialCompleted={Boolean(progressLookup.get(item.uuid ?? ''))}
										/>
									</div>
