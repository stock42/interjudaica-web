...
export async function GET(
  ...
  const item = await CourseStorage.get(uuid);
...
export async function PATCH(
  ...
  try {
    const { uuid } = await params;
    const payload = schemaCourse.partial().parse(await readJson(request));
    const item = await CourseStorage.update(uuid, payload);
