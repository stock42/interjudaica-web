...
export async function GET(
	...
	const item = await EmailGroupStorage.get(uuid)
...
export async function PATCH(
	...
	try {
		const { uuid } = await params
		const payload = schemaEmailGroup.partial().parse(await readJson(request))
		const item = await EmailGroupStorage.update(uuid, payload)
		if (!item) return NextResponse.json({ error: 'Not found' }, { status: 404 })
		return NextResponse.json({ item })
	} catch (error) {
