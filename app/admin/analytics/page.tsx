...
export default async function AdminAnalyticsPage() {
	...
	const subscriptionManagedUsers = communityUsers.filter(user =>
		Boolean(user.stripeSubscriptionId),
	)
	const courseRevenue = paidCoursePayments.reduce(
		(total, payment) => total + payment.amount,
		0,
	)
	const bookRevenue = paidBookSales.reduce((total, sale) => total + sale.amount, 0)
	...
	return (
		...
		>
			<div className="grid gap-5 lg:grid-cols-3">
				{[
					[
						'Paid course payments',
						String(paidCoursePayments.length),
						`${payments.length} total records`,
					],
					[
						'Active community users',
						String(activeCommunityUsers.length),
						`${subscriptionManagedUsers.length} managed by Stripe`,
					],
					['Recent errors', String(errors24h), `${errors7d} in the last 7 days`],
					[
						'Revenue tracked',
						`$${(courseRevenue + bookRevenue).toFixed(2)}`,
						'Course and book payments',
					],
				...
				<DataTable
					...
					rows={[
						[
							'Course payments',
							String(paidCoursePayments.length),
							String(payments.length - paidCoursePayments.length),
							`${Math.round((paidCoursePayments.length / Math.max(payments.length, 1)) * 100)}% paid`,
						],
						[
							'Payment attention',
							String(pendingPayments.length),
							String(failedPayments.length),
							'pending / failed',
						],
						[
							'Book sales',
							String(paidBookSales.length),
							String(bookSales.length - paidBookSales.length),
							`${Math.round((paidBookSales.length / Math.max(bookSales.length, 1)) * 100)}% paid`,
						],
						[
							'Forum threads',
							String(forums.length),
							String(forums.filter(thread => thread.status === 'open').length),
							`${forums.filter(thread => thread.area === 'Community Forum').length} community`,
						],
