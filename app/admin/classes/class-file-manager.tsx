...
export function ClassFileManager({
	courseUuid,
	classUuid,
}: ClassFileManagerProps) {
	...
	useEffect(() => {
		...
		async function loadFiles() {
			...
			const items = Array.isArray(itemsValue)
				? itemsValue.filter(isCourseClassFile)
				: [];
			setFiles(items);
			...
			<div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
				<div>
					...
					<p className="mt-1 text-sm leading-6 text-[var(--muted)]">
						Upload any file type. Enrolled students download these files from
						their purchased course page.
					</p>
				</div>
			...
			<div className="mt-5 grid gap-3">
				{loading ? (
					...
					<p className="rounded-lg border border-dashed border-[var(--line)] bg-[var(--paper)] p-5 text-sm leading-6 text-[var(--muted)]">
						No class materials yet. Add readings, recordings, slides, ZIP files,
						or any other course resource.
					</p>
				)}
			</div>
		</section>
