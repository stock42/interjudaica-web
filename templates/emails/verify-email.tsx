import * as React from "react";

import { base, card, h1, palette, small } from "@/templates/emails/_styles";

const LOGO_URL =
	"https://interjudaica.com/_next/image?url=%2Flogo-interjudaica-transparente.png&w=1920&q=75";

export default function VerifyEmail({
	firstName,
	code,
}: {
	firstName: string;
	code: string;
}) {
	return (
		<div style={base}>
			<div style={card}>
				<div style={{ display: "flex", alignItems: "center", gap: 14 }}>
					<img
						src={LOGO_URL}
						width={56}
						height={56}
						alt="InterJudaica"
						style={{
							borderRadius: 9999,
							border: `1px solid ${palette.line}`,
						}}
					/>
					<div>
						<p
							style={{
								margin: 0,
								color: palette.gold,
								fontWeight: 800,
								letterSpacing: "0.14em",
								textTransform: "uppercase",
								fontSize: 12,
							}}
						>
							InterJudaica
						</p>
						<p style={{ ...h1, margin: "6px 0 0" }}>
							Verify your email
						</p>
					</div>
				</div>

				<div
					style={{
						marginTop: 18,
						color: palette.muted,
						fontSize: 14,
						lineHeight: "22px",
					}}
				>
					<p style={{ margin: 0 }}>Hi {firstName},</p>
					<p style={{ margin: "10px 0 0" }}>
						Use the code below to verify your email and activate your
						InterJudaica account.
					</p>
				</div>

				<div
					style={{
						marginTop: 18,
						border: `1px solid ${palette.line}`,
						borderRadius: 14,
						padding: 16,
						textAlign: "center",
						backgroundColor: "rgba(0,0,0,0.28)",
					}}
				>
					<p
						style={{
							margin: 0,
							fontSize: 28,
							letterSpacing: "0.3em",
							fontWeight: 800,
							color: palette.gold,
						}}
					>
						{code}
					</p>
				</div>

				<p style={{ marginTop: 18, ...small }}>
					This code expires in 20 minutes. If you did not create an
					InterJudaica account, you can safely ignore this email.
				</p>
			</div>
		</div>
	);
}
